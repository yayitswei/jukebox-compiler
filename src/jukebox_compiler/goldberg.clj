...
(defn zip-song-to-play [{:keys [pitches durations]}]
  (let [times (reductions + 0 durations)]
    (map vector times pitches)))

(defn zip-song-for-compilation [{:keys [pitches durations]}]
  (map vector durations pitches))
...
(def braid
  {:pitches
   [0 4 2 1 0 -1 0 1 -2 -3
    0 4 2 1 0 3 4 5 6]
   :durations
   [2 1 3/2 1/2 1 3/2 1/2 1 1 2
    2 1 3/2 1/2 1 3/2 1/2 1 1 4]})

(comment
(play
  (map
    (fn [[beat degree]]
      [((bpm 180) beat) ((comp C major) degree)])
    (zip-song braid))
  )
)
...
(play (=>
        (zip-song-to-play braid)
        (in-key (comp E major))
        (in-time (bpm 130))))

(map (fn [[x y]] (- y x)) (partition 2 1 [1 2 3 4]))

(defn transform-note-for-arduino [[duration midi]]
  (str midi "," duration))

(defn compile-for-arduino [notes]
  (string/join " " (map transform-note-for-arduino notes)))

(compile-for-arduino
  (=> melody (in-key (comp F major)) (in-time (bpm 180))))

(compile-for-arduino
  (=> (zip-song-for-compilation braid)
      (in-key (comp E major))
      (in-time (bpm 130))))
